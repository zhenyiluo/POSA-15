...
/**
 * An IntentService that downloads an image requested via data in an intent,
 * stores the image in a local file on the local device, and returns the image
 * file's URI back to the MainActivity via the Messenger passed with the intent.
 */
public class DownloadImageService extends IntentService {
	...
	/**
	 * String constant used to extract the pathname to a downloaded image from a
	 * Bundle.
	 */
	...
	/**
	 * String constant used to extract the directory pathname to use to store a
	 * downloaded image.
	 */
	...
	/**
	 * Helper method that returns the path to the image file if it is download
	 * successfully.
	 */
	...
	/**
	 * Helper method that returns the request code associated with the @a
	 * message.
	 */
	...
	/**
	 * Hook method dispatched by the IntentService framework to download the
	 * image requested via data in an intent, store the image in a local file on
	 * the local device, and return the image file's URI back to the
	 * MainActivity via the Messenger passed with the intent.
	 */
	@Override
	public void onHandleIntent(Intent intent) {
		...
		// @@ You fill in here.
		String directoryPathname = intent.getExtras().getString(
				DIRECTORY_PATHNAME);

		// Download the requested image.
		// @@ You fill in here.
		Uri pathToImageFile = Utils.downloadImage(DownloadImageService.this,
	...
	private void sendPath(Messenger messenger, Uri pathToImageFile, Uri url) {
		...
		try {
			...
			Log.e(TAG, "Exception while sending.", e);
		}
	}

	/**
	 * A factory method that creates a Message to return to the MainActivity
	...
	private Message makeReplyMessage(Uri pathToImageFile, Uri url) {
		...
		// IMAGE_URL key.
		// @@ You fill in here.
		data.putParcelable(IMAGE_URL, url);

		// Return the result to indicate whether the download
		// succeeded or failed.
		// @@ You fill in here.
		message.arg1 = pathToImageFile == null ? Activity.RESULT_CANCELED
				: Activity.RESULT_OK;
		...
		if (message.arg1 == Activity.RESULT_OK) {
			data.putParcelable(IMAGE_PATHNAME, pathToImageFile);
		}
